  ...
  request.query(chkquery,function(err,chkqueryrecords){
    ...
    console.log(err);
       ...
       // }
        ...
        //console.log(existingDeviceId.length)
        query = ""
        newRecordAdded = 0
        ...
        for ( i=0; i<devices.length;i++ ){
               ...
               /// console.log(devices[i]['deviceid'])
                ...
                if(existingDeviceId.includes(devices[i]['deviceid']) == false){
                 ...
                 // console.log(newRecordAdded +" the id is "+devices[i]['deviceid'])
...
//        console.log(query)
        if(newRecordAdded > 0){
          console.log('IF is running for all new record added')
        request.query(query,function(err,records){ // if no devices in the device table with the given device id then add a device here
          if(err)
          console.log(err);
          else{
            ...
            //query2 = "select TOP ("+devices.length+") recordid FROM ["+dbName+"].[ECCAnalytics].[Devices] where deviceid = '"+deviceid+"' order by recordid DESC;"
           // query2 = "select TOP ("+newRecordAdded+") recordid FROM ["+dbName+"].[ECCAnalytics].[Devices] where deviceid = '"+deviceid+"' order by recordid DESC;"
            query2 = "select TOP ("+newRecordAdded+")  recordid, deviceid FROM ["+dbName+"].[ECCAnalytics].[Devices] where equipmentname = '"+req.body.equipmentname+"' order by recordid DESC;"
            request.query(query2,function(err,records){
              if(err)
              console.log(err);
              else{
               ...
               request.query(query3,function(err,records){
                if(err){
                console.log(err);
                return res.status(200).json('failed2')
    ...
    else{   //Else case for  device is already added to device table
