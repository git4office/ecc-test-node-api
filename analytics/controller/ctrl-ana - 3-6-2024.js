  ...
  const dashboardlogout = (req,res)=>{
    ...
    sql.connect(config,function(err){
      ...
      // query = "update ["+dbName+"].ECCAnalytics.Users set [loginstatus] = 0 where  username ='"+username+ "';"
       query = "update ["+dbName+"].ECCAnalytics.Users set [analyticloginstatus] = 0 where  username  COLLATE SQL_Latin1_General_CP1_CS_AS ='"+username+ "';"
       query += "update  ["+dbName+"].ECCAnalytics.UserLog set [logouttime] = CURRENT_TIMESTAMP where  username  COLLATE SQL_Latin1_General_CP1_CS_AS ='"+username+"' and app = 2 and logouttime IS NULL;"
