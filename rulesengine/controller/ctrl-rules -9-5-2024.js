...
return;
  ...
  query = "SELECT  [ruleid] FROM ["+dbName+"].[ECCAnalytics].[Rules] where ruleid ='"+ruleid+"';"
...
const updatetimerrecord = (req,res)=>{
  console.log(req.originalUrl);
...
const createtimerrecord = (req,res)=>{
  console.log(req.originalUrl);
            ...
            if(records['recordsets'][0][0].total == 0){
              ...
              request.query(query,function(err,records){
                ...
                console.log(err);
                    ...
                    return res.status(200).json({'timer': records['recordsets'][0][0].timer})
...
const getprojectfieldsforrules = (req,res)=>{
  ...
  sql.connect(config,function(err){
     ...
     var query = "SELECT buildingname,equipmentname,equipmentid,associatedequipid  FROM  ["+dbName+"].[ECCAnalytics].[Project]"
